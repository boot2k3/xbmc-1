  ...
  while (!m_bStop)
  {
    ...
    else if (ret == MSGQ_TIMEOUT)
    {
          ...
          m_picture.videoBuffer)
      {
		...
		CLog::Log(LOGDEBUG, "00 MSGQ_TIMEOUT OUTPUT_AGAIN 01 vfmt attempt frame_format: %s", vfmt);
        m_outputSate = OutputPicture(&m_picture);
		if (!SysfsUtils::GetString("/sys/class/deinterlace/di0/frame_format", vfmt) && (vfmt.size() > 4))
		{
			m_processInfo.SetVideoInterlaced(vfmt.compare("progressive"));
		}
		CLog::Log(LOGDEBUG, "00 MSGQ_TIMEOUT OUTPUT_AGAIN 02 vfmt attempt frame_format: %s", vfmt);
        if (m_outputSate == OUTPUT_AGAIN)
        {
          onlyPrioMsgs = true;
		  if (!SysfsUtils::GetString("/sys/class/deinterlace/di0/frame_format", vfmt) && (vfmt.size() > 4))
		  ...
		  CLog::Log(LOGDEBUG, "00 MSGQ_TIMEOUT OUTPUT_AGAIN 03 vfmt attempt frame_format: %s", vfmt);
          continue;
        }
      }
