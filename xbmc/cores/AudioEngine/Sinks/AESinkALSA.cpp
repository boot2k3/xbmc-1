...
void CAESinkALSA::aml_configure_simple_control(std::string &device, const enum IEC958_mode_codec codec)
{
  ...
  if (cardNr >= 0)
  {
    ...
    CLog::Log(LOGINFO, "CAESinkALSA - Use card \"%s\" and set codec format \"%s\"", card, AMLCodecToStr(codec).c_str());
    ...
    {
      CLog::Log(LOGERROR, "CAESinkALSA- can not open Mixer: %s\n", snd_strerror(err));
      return;
    }

    if ((err = snd_mixer_attach(handle, card)) < 0)
    {
      CLog::Log(LOGERROR, "CAESinkALSA - Mixer attach %s error: %s", card, snd_strerror(err));
    ...
    {
      CLog::Log(LOGERROR, "CAESinkALSA - Mixer register error: %s", snd_strerror(err));
      snd_mixer_close(handle);
      return;
    }
    ...
    {
      CLog::Log(LOGERROR, "CAESinkALSA- Mixer %s load error: %s", card, snd_strerror(err));
      snd_mixer_close(handle);
      return;
    }
    ...
    {
        CLog::Log(LOGINFO, "CAESinkALSA - Set codec for \"%s\"", sid_name.c_str());
        snd_mixer_selem_id_set_name(sid, sid_name.c_str());
        elem = snd_mixer_find_selem(handle, sid);
        if (!elem) {
          CLog::Log(LOGERROR, "CAESinkALSA - Unable to find simple control '%s',%i\n",
    ...
    CLog::Log(LOGINFO, "CAESinkALSA - Set Spdif to HDMITX to \"%s\"", AMLSpdifIDToStr(spdif_id).c_str());
...
bool CAESinkALSA::Initialize(AEAudioFormat &format, std::string &device)
{
  ...
  if (amlDeviceType != AML_NONE)
  {
    ...
    CLog::Log(LOGINFO, "CAESinkALSA::Initialize - Configure simple control for \"%s\"",
      GetAMLCardName(amlDeviceType).c_str());
